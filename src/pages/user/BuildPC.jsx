...
export default function BuildPC() {
  ...
  const loadAllComponents = async () => {
    ...
    try {
      ...
      // Keyword-based filtering on tenSP (API has no danhMuc)
      const caseData = allData.filter(
        (item) =>
          item.tenSP?.toLowerCase().includes("case") ||
          item.tenSP?.toLowerCase().includes("vỏ"),
      );

      setCpus(allData.filter((item) => checkKeyword(item, /intel|ryzen|cpu/i)));
      setMainboards(
        allData.filter((item) =>
          checkKeyword(item, /mainboard|b\d+|h\d+|z\d+|x\d+/i),
        ),
      );
      setRams(allData.filter((item) => checkKeyword(item, /ram|ddr[45]/i)));
      setGpus(
        allData.filter((item) =>
          item.tenSP?.toLowerCase().match(/rtx|gtx|rx|vga|gpu/),
        ),
      );
      setSsds(
        allData.filter((item) => item.tenSP?.toLowerCase().match(/ssd|nvme/)),
      );
      setPsus(
        allData.filter((item) =>
          item.tenSP?.toLowerCase().match(/psu|nguồn|rm\\d+|hx\\d+|80\\+/),
        ),
      );
      setCases(
        allData.filter((item) =>
          item.tenSP?.toLowerCase().match(/lancool|lian li|case|vỏ|tower/),
        ),
      );

      // Debug logs - Updated for tenSP regex filters
      console.log(
        "CPUs:",
        allData.filter((item) =>
          item.tenSP?.toLowerCase().match(/intel|ryzen|cpu/),
        ).length,
      );
      console.log(
        "Mainboards:",
        allData.filter((item) =>
          item.tenSP?.toLowerCase().match(/mainboard|b760|b660|h610|z790|x670/),
        ).length,
      );
      console.log(
        "RAMs:",
        allData.filter((item) =>
          item.tenSP?.toLowerCase().match(/ram|ddr4|ddr5/),
        ).length,
      );
      console.log(
        `CPUs: ${cpus.length}, Mainboards: ${mainboards.length}, RAMs: ${rams.length}, GPUs: ${gpus.length}, SSDs: ${ssds.length}, PSUs: ${psus.length}, Cases: ${cases.length}`,
      );
